...
def get_cmd():
    ...
    parser.add_argument("-d", "--dataset", default="Youshu", type=str, help="which dataset to use, options: Youshu, NetEase, iFashion")
...
def main():
    ...
    for lr, l2_reg, UB_ratio, UI_ratio, BI_ratio, embedding_size, num_layers, c_lambda, c_lambda_2, alpha_1, alpha_2, hyper_num, h_m, hyper_ratio, c_temp in \
            product(conf['lrs'], conf['l2_regs'], conf['UB_ratios'], conf['UI_ratios'], conf['BI_ratios'], conf["embedding_sizes"], conf["num_layerss"], conf["c_lambdas"], conf["c_lambdas_2"], conf["alpha_1"], conf["alpha_2"], conf['hyper_num'], conf['h_m'], conf['hyper_ratio'], conf["c_temps"]):
        ...
        conf["c_lambda_2"] = c_lambda_2
        conf["alpha_1"] = alpha_1
        conf["alpha_2"] = alpha_2
        conf["c_temp"] = c_temp
        conf['hyper_num'] = hyper_num
        conf['h_m'] = h_m
        conf['hyper_ratio'] = hyper_ratio
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if conf["aug_type"] == "ED" and (batch_anchor + 1) % ed_interval_bs == 0:
                    ED_drop = True
                bpr_loss, c_loss, ub_c_loss = model(batch, ED_drop=ED_drop)
                loss = bpr_loss + c_lambda * c_loss + c_lambda_2 * ub_c_loss 
                loss.backward()
                optimizer.step()
                ...
                pbar.set_description("epoch: %d, loss: %.4f, bpr_loss: %.4f, c_loss: %.4f" %(epoch, loss_scalar, c_loss_scalar, bpr_loss_scalar))
...
def test(model, dataloader, conf):
    ...
    device = conf["device"]
    model.eval()
    users_feature, bundles_feature, _, _, _, _ = model.get_multi_modal_representations(test=True)
    for users, ground_truth_u_b, train_mask_u_b in dataloader:
        pred_b = model.evaluate(users_feature, bundles_feature, users.to(device))
